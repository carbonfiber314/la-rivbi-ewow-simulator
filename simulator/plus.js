...
let dnp_enabled = true;
let dnp_toggle = document.getElementById("dnp_toggle");

function dnp_toggler() {
	dnp_enabled = !dnp_toggle.checked;
}

dnp_toggle.addEventListener("change", dnp_toggler);
dnp_toggler(); // Some browsers save settings, in that case it must double-check - carbon

let ld2_enabled = true;
let ld2_start = 16;

let ld2_prize = 0.10;
let ld2_safe = 0.66;

let ld2_toggle = document.getElementById("ld2_toggle");
let ld2_start_select = document.getElementById("ld2_start");
let ld2_prize_select = document.getElementById("ld2_prize");
let ld2_safe_select = document.getElementById("ld2_safe");


function ld2_updater() {
	ld2_enabled = ld2_toggle.checked;
	ld2_start = ld2_start_select.value;
	ld2_prize = ld2_prize_select.value/100;
	ld2_safe = ld2_safe_select.value/100;
}

ld2_toggle.addEventListener("change", ld2_updater);
ld2_start_select.addEventListener("change", ld2_updater);
ld2_prize_select.addEventListener("change", ld2_updater);
ld2_safe_select.addEventListener("change", ld2_updater);
...
function reset_simulation(){
	...
	for (let i = 0; i < contestants.length; i++){
		...
		lives_element.style.backgroundColor = `var(--${[c.lives]}-lives-color)`;
		if(c["rank" + total_eps] <= prize_ranks[total_eps]){
...
document.getElementById("reset_sim").addEventListener("click", reset_simulation);


function z_to_pertentile(z){
	// TODO
	// I'm not doing this btw - carbon
}
...
function simulate_round () {
	...
	for (let i = 0; i < contestants.length; i++){
		...
		} else if (Math.random() < c.dnp_rate && dnp_enabled){ // the contestant DNP'd
	...
	let safe_spots = Math.round(response_count * (current_episode >= ld2_start && ld2_enabled ? ld2_safe : 0.51));
	...
	for (let i = 0; i < contestants.length; i++){
		...
		// life decay 2
		if(current_episode >= ld2_start && c.lives > 1 && ld2_enabled) {
			c.lives -= 1
			c.responses += 1
		}
